from .models import User,Profile,Token,Grade,subject
import uuid
from . serializer import UserSerializer
from mongoengine import connect,get_db
connect(host='mongodb+srv://root:12345@cluster0.tnb7vm7.mongodb.net/school')
db=get_db()
...
def sub():
    # try:
        data = request.json
        print(data)
        grade = Grade(grade='1',section='c')
        print(grade)
        grade.save()
        Subject = subject(name=data['name'],code=data['code'],created_at=data['created_at'],grade=grade)
        print(Subject)
        Subject.save()
        return Response(dumps({'message':'successfull'}),status=200)
    # except Exception as e :
    #     return Response(dumps({"message":'unsuccessfull'}),status=404)    
